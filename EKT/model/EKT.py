...
class EKTM_dev(nn.Module):
    def __init__(self, knowledge_length, knowledge_emb_size, seq_hidden_size, is_text=False, text_emb_size=None, gpu=False):
        '''
        @ param:
        ...
        if self.score_mode == "double":
            self.rnn = nn.GRU(self.text_emb_size * 2, self.seq_hidden_size, num_layers=1)

        self.h_initial = nn.Parameter(torch.zeros(self.knowledge_length, self.seq_hidden_size))
        self.h_initial.data.uniform_(-1, 1)

        #prediction layer
        self.score_layer1 = nn.Linear(self.text_emb_size + self.seq_hidden_size, 50) # x_t+1, s_t+1
        self.relu = nn.ReLU(inplace=True)
        self.score_layer2 = nn.Linear(50, 1)
    ...
    def forward(self, x, hidden=None, text_token=None, beta=None):
        ...
        '''
        label = 0 if x <= self.knowledge_length else 1
        x = (x-1) % self.knowledge_length + 1
        # print(x, label)
        ...
        if self.is_text:
            if self.gpu:
                text_token = self.fix_text_token.cuda()
            else:
        ...
        if self.gpu:
            concat_s = torch.cat([text_token, s]).view(1, -1).cuda()
        else:
            concat_s = torch.cat([text_token, s]).view(1, -1)
        prediction_score = self.relu(self.score_layer1(concat_s))
        prediction_score = self.sigmoid(self.score_layer2(prediction_score))

        if self.score_mode == "double":
            zero_token = torch.zeros_like(text_token)
            if label >= 0.5:
                x_hat = torch.cat([text_token, zero_token])     # 和原文反了，已修改
            else:
