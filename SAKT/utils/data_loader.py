        ...
        for lineID, line in enumerate(f_data):
            ...
            if lineID % 3 == 1:
                ...
                if len(q_ids) >= self.min_len:
                    ...
                    target_index_vec = self.padding_list_to_fix_length(target_index_vec, self.max_len)
                    target_response_vec = self.padding_list_to_fix_length(target_response_vec, self.max_len)
                    encoded_data.append(encoded_vec)
                    target_index_data.append(target_index_vec)
                    target_response_data.append(target_response_vec)
...
data = DataLoader(",", 10, 1, 0)
print(data.scan_file("../data/test.csv"))
# q = [0, 1, 2, 3, 4]
# r = [1, 0, 0, 1, 0]
...
print(data.prepare_model_data("../data/test.csv", 7))
